					...
					{{tariffId.name}}
						<DialogClose>
							<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40"
								fill="none">
								<path d="M27.0708 12.9288L12.9287 27.0709M27.0708 27.0708L12.9287 12.9287"
									stroke="#020105" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
							</svg>
						</DialogClose>
					</DialogTitle>
				</DialogHeader>
						...
						<div class="grid gap-6 grid-cols-1 lg:grid-cols-2">
							<div class="grid gap-2 relative">
								<VField name="name" rules="required" v-model="form.name">
									<Input v-model="form.name" id="login" type="text" :placeholder="translations['form.your name']"
							...
							<div class="grid gap-2 relative">
								<VField name="staff" rules="required" v-model="form.staff">
									<Input @keypress="isNumber($event)" v-model="form.staff" id="staff" type="text"
										:placeholder="translations['form.employees_input']"
										class="border-0 border-b rounded-none py-3 px-0 placeholder:text-grey-1"
							...
							<div class="grid gap-2 relative">
								<VField name="company">
									<Input v-model="form.company" id="company" type="text"
										:placeholder="translations['form.company_input']"
										class="border-0 border-b rounded-none py-3 px-0 placeholder:text-grey-1" />
								</VField>
							</div>
							<div class="grid gap-2 relative">
								<VField name="name">
									<Input v-model="form.owner" id="owner" type="text" 	:placeholder="translations['form.position_input']"
										class="border-0 border-b rounded-none py-3 px-0 placeholder:text-grey-1" />
								</VField>
							</div>
							<div class="grid gap-2 relative">
								<VField name="phone" rules="required" v-model="form.phone">
									<div class="relative">
										...
										<span
											class="absolute start-0 inset-y-0 flex items-center justify-center text-black">+998</span>
									</div>
								</VField>
							</div>
							<div class="grid gap-2 relative">
								<VField name="email" type="email" rules="required|email" v-model="form.email">
									<Input v-model="form.email" id="email" type="text"
										:placeholder="translations['form.email_input']"
										class="border-0 border-b rounded-none py-3 px-0 placeholder:text-grey-1"
						...
						<div class="flex justify-end mt-6">
							<Button class="bg-[#1878F3] text-white" :disabled="loading">
								...
								<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
									fill="none">
									<path d="M11.6667 6.6665L15 9.99984M15 9.99984L11.6667 13.3332M15 9.99984L5 9.99984"
